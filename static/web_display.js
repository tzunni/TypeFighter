...
class UI_Handler{
    ...
    display_book_cover(book){
        ...
        let book_location = {
            "COVERS":"book_information",
            "COVER_I":"search_result"
        }
        document.getElementById(book_location[this.book_handler.get_book_id_type(book)]).appendChild(image)
    }
    ...
    display_search_result(search_result){
        const _field_names = this.book_handler.get_book_field_names(search_result);
        const _missing_field_names = this.book_handler.get_book_missing_field_names(search_result);
        let output_string = ""
        if (this.book_handler.is_missing_field_data(search_result)){

            let error_fields = _missing_field_names.join(", ");
            document.getElementById("search_status").innerHTML = "Status - Missing the following fields: " + error_fields;
            document.getElementById("search_status").style.color = this.status_colours.RED;
        }
        else{
            document.getElementById("search_status").innerHTML = "Status: OK";
            document.getElementById("search_status").style.color = this.status_colours.GREEN;
        }
        ...
        for (let i = 0; i < _field_names.length; i++){
            ...
            if (_missing_field_names.includes(_current_field_name)){
                ...
                output_string += (_current_field_name + ": " + this.book_handler.get_book_field_data(search_result, _current_field_name) + "<br />");
            }
        }
        // To Do (BUG): 0 will encode for null, such as ratings_count_1 = 0, will be set to null when output. Likely to do with missing field names
        document.getElementById("search_result").innerHTML = output_string;
    }
} // End Class
